...
async function print() {
  ...
  const cs = await Promise.all(commitments.map(async c => {
    ...
    if (encoded_messages.length === 0) {
      ...
      encoded_messages = await getOffchainDataForCommitment(
        appchain,
        key
      );
    }
    const decoded_messages: any = decodeMessages(
      encoded_messages
    );
  ...
  console.log(util.inspect(decoded, { showHidden: false, depth: null, colors: true, 'maxArrayLength': null }));
  console.log("==============================");
}
