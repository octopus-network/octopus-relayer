...
import { tryComplete, checkAnchorIsWitnessMode } from './nearCalls'
...
export async function confirmAction(
  ...
  try {
    ...
    if (e.transaction_outcome) {
      ...
      console.error('tryCompleteActions failed-txId', e.transaction_outcome.id)
    }
  }
  return returnVal
}
...
export async function isActionHealthy(type: ActionType) {
  ...
  if (
    !action ||
    !action.failed_at ||
    Date.now() - action.failed_at > updateStateMinInterval * MINUTE
  ) {
    return true
  } else {
    return false
